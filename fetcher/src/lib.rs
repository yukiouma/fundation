...
pub use dto::{Nav, SaveNavsRequest};
...
mod tests {
    ...
    fn it_works() {
        ...
        for code in fond_codes {
            let url = builder.url(code, 1, 20).unwrap();
            ...
            let resp = fetcher.fetch(&url).unwrap();
            ...
            loop {
                ...
                let url = builder.url(code, page, pagesize).unwrap();
                let resp = fetcher.fetch(&url).unwrap();
                let navs = resp
                    .data
                    .value_list
